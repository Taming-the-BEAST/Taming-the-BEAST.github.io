...
\begin{center}
	...
	\textbf{\LARGE Tutorial using BEAST v2.5.x}\\\vspace{2mm}
...
phylogenetic trees. This tutorial is written for BEAST v2.5.x \citep{Bouckaert2014, Bouckaert2019}.
...
to test the performance of skyline methods \citep{Drummond2005}
\citep{Stadler2013}.
...
downloading the file \lstinline!hcv.nexus! from the left-hand panel,
under the heading \textbf{Date}.
...
in the menu and then finding the \lstinline!hcv.nexus! file on your
computer \textbf{or} simply drag and drop the file into the
\textbf{BEAUti} window.
\end{framed}
...
\lstinline!*.nexus! file) and open a dialogue box to confirm the data
type.
...
as \textbf{C} to \textbf{A} etc. Additionally, we allow for rate
...
\textbf{bGroupSizes} (note that the dimension of both parameters has to
...
which means $ N_e $ will be allowed to change 3 times
between the tMRCA and the present (if we have $ d $
segments, $ N_e $ is allowed to change $ d-1 $
times).
...
\textbf{File Name} to \lstinline!$(filebase).log! and set the
\textbf{Log Every} to 3'000.

Click on the arrow next to the \textbf{treelog} and change the
\textbf{File Name} to \lstinline!$(filebase).$(tree).log! and set the
\textbf{Log Every} to 3'000.

Leave all other settings at their default values and save the file as
\lstinline!hcv_coal.xml!.
\end{framed}
...
name of the XML file. This is a good idea, since it makes it easy to
keep track of which XML files produced which output files.

Now we are ready to run the analysis.

\begin{framed}
Start \textbf{BEAST2} and choose the file \lstinline!hcv_coal.xml!.
...
The coalescent model that the Coalescent Bayesian Skyline is based on
...
see Figure \ref{fig:coal_events}). Grouping segments together leads to
...
specified (Skyride \citep{Minin2008}), but instead makes very strong
prior assumptions about changes in $ N_e $.
...
file contains the information about the group sizes and population sizes
of each segment, while the trees file is needed for the times of the
coalescent events.
...
(\lstinline!hcv_coal_30M.log!).

\begin{framed}
Navigate to \textbf{Analysis \textgreater{} Bayesian Skyline
Reconstruction}. From there open the \lstinline!*.trees! file. To get
the correct dates in the analysis we should specify the \textbf{Age of
the youngest tip}. In our case it is 1993, the year where all the
samples were taken. If the sequences were sampled at different times
(heterochronous data), the age of the youngest tip is the time when the
most recent sample was collected.
...
(Note that the reconstruction will only work if the \lstinline!*.log!
and \lstinline!*.trees! files contain the same number of states and both
files were logged at the same frequency).
...
\lstinline!*.pdf! for display purposes or as a tab delimited file.
...
\begin{framed}
Restart \textbf{BEAUti}, load \lstinline!hcv.nexus! as before and set up
the same site and clock model as in the Coalescent Bayesian Skyline
analysis.
\end{framed}
...
the dimension for $ R_e $, the effective reproduction
number, which denotes the average number of secondary infections caused
...
This means that $ R_e $ will be allowed to change at 9 times
equally spaced between the origin of the epidemic and the present time.
Choosing this dimension can again be arbitrary and may require the
...
contain enough information to infer parameters. (As with setting the
dimension of the Coalescent Bayesian Skyline the dimension of
$ R_e $ can also be set in the initialization panel).

Besides $ R_e $ (\textbf{reproductiveNumber}), the
\textbf{Birth Death Skyline Contemporary} model has 3 more parameters,
\textbf{becomeUninfectiousRate} (the rate at which infected patients
become uninfectious, $ \delta $, through recovery, death or
isolation), \textbf{rho} (the proportion of lineages sampled in the
present, $ \rho $) and the \textbf{origin} (the time at
...
of the most infectious diseases we know about and has
$ R_e \approx 18 $). If an epidemic is neither growing or
declining, it has an $ R_e $ of 1, which we will use as a
null hypothesis, by setting a prior on $ R_e $ centered
around 1 (we assume that if there isn't a strong signal in an interval
for an epidemic to grow or decline that $ R_e = 1 $,
i.e.~the epidemic size stays constant). Note that this prior is used for
...
all the options for $ R_e $ settings

Set \textbf{M} to 0, which results in a median of 1. We set the variance
...
we used for the $ R_e $. In terms of the becoming
uninfectious rate, this translates to the 95\% quantiles for the
infectious period falling between 0.128 years (46.67 days) and 11.59
...
The sampling proportion, $ \rho $, represents the proportion
of HCV cases in Egypt in 1993 that are included in the analysis. In 1993
Egypt had a population of roughly 60 million people, and with a
prevalence of at least 15\% this translates into millions of cases,
while we only have 63 sequences.

We will use a beta distribution for the prior on $ \rho $.
Beta distributions are a very flexible class of distributions that are
only defined between 0 and 1, making them ideal to use for proportions.
...
Leave all other settings at their default values and save the file as
\lstinline!hcv_bdsky.xml!.
\end{framed}

Now we are ready to run the analysis.

\begin{framed}
Start \textbf{BEAST2} and choose the file \lstinline!hcv_bdsky.xml!.
...
\item
  We set a prior on $ R_e $ in the Birth-Death Skyline
  analysis, but did not set any prior for $ N_e $ in the
  Coalescent Bayesian Skyline analysis. Is there a prior on
  $ N_e $? If so, what is it?
\item
...
an epidemic will keep growing. We can see this from the definitioin of
...
$ R_e $ is often more informative for prevention efforts. In
addition, the model also has a sampling probability
($ \rho $) parameter, which in our analysis describes how
likely it is that a person infected with HCV in Egypt in 1993 was
sampled in our dataset. The final parameter is the origin. Whereas
...
$ \lambda, \delta $ and $ \rho $ and is usually
more appropriate for macro-evolutionary studies. The \textbf{Birth Death
Skyline Serial} model assumes that the data are heterochronous (sampled
...
that case a parameterization in terms of $ \lambda, \mu $
...
mixing or biased estimates (often we assume that the becoming
uninfectious rate is constant while allowing $ R_e $ to
change over time, as we did here). It is also possible to specify the
...
the origin parameter, the interval times are slightly different in each
...
do some extra post-processing to plot the smooth skyline.
...
settings and a chain of 30,000,000 (\lstinline!hcv_bdsky_30M.log!).
...
We will use the R-package \lstinline!bdskytools! to plot the output of
the BDSKY analysis. The package is still in development and not
available over CRAN. Thus, we have to install the package directly over
GitHub (note that you only have to install the package once):
...
Once the package is installed we have to load it into our R workspace
before we can use the functions in the package. To plot the results, we
need to first tell R where to find the \lstinline!*.log! file of our run
and then load it into R (discarding 10\% of samples as burn-in). If you
are using RStudio, you can change the working directory to the directory
...
(\lstinline!timegrid!), refers to years in the past.
...
different posterior samples. This is because the origin is different in
...
Finally, we can plot both the $ R_e $ and
$ \delta $ (the becoming uninfectious rate) on a single set
of axes. Since we left the dimension of the becoming uninfectious rate
at 1, it is constant through time. (Normally we would not plot constant
parameters over a time period)! The output should be similar to Figure
\ref{fig:bdsky_out}.
